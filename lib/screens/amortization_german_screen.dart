...
class AmortizationGermanScreen extends StatefulWidget {
  @override
  _AmortizationGermanScreenState createState() =>
      _AmortizationGermanScreenState();
}

class _AmortizationGermanScreenState extends State<AmortizationGermanScreen> {
  ...
  double? _fixedAmortization;
  double? _initialInterest;
  double? _firstPayment;
  ...
  void _calculateGermanAmortization() {
    if (_formKey.currentState!.validate()) {
      ...
      double fixedAmortization = principal / totalPeriods;

      List<Map<String, dynamic>> table = [];
      double remainingDebt = principal;

      double initialInterest = remainingDebt * periodRate;
      double firstPayment = fixedAmortization + initialInterest;

      for (int i = 1; i <= totalPeriods; i++) {
        double interest = remainingDebt * periodRate;
        double payment = fixedAmortization + interest;

        table.add({
          'period': i,
          'payment': payment,
          'capital': fixedAmortization,
          'interest': interest,
          'remainingDebt': remainingDebt - fixedAmortization,
        });

        remainingDebt -= fixedAmortization;
      }

      setState(() {
        _amortizationTable = table;
        _showTable = true;
        _fixedAmortization = fixedAmortization;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text(
            'Se caracteriza por cuotas decrecientes en el tiempo, con amortización del capital fija y los intereses disminuyen progresivamente.',
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Ideal para empresas y financiamientos a corto plazo',
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: 13,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildKeyValueItem(
            ...
            value: '${_fixedAmortization?.toStringAsFixed(2) ?? "0.00"}',
            bgColor: Colors.purple[100]!,
            icon: Icons.analytics,
          ),
          SizedBox(height: 12),
          _buildKeyValueItem(
            label: 'Intereses iniciales',
            value: '${_initialInterest?.toStringAsFixed(2) ?? "0.00"}',
          ...
          _buildKeyValueItem(
            label: 'Primera cuota',
            value: '${_firstPayment?.toStringAsFixed(2) ?? "0.00"}',
              ...
              rows: _amortizationTable.map((row) {
                return DataRow(
                  cells: [
                    ...
                    DataCell(Text('${row['remainingDebt'].toStringAsFixed(2)}')),
                  ],
                );
              }).toList(),
            ),
